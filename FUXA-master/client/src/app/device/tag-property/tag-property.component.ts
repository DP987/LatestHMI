...
export class TagPropertyComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        if (this.isOpcua() || this.isBACnet() || this.isWebApi() || this.isOdbc()) {
            ...
            if (this.isModbus()  || this.isModbusTCPdll() ) {
    ...
    onOkClick(): void {
        ...
        } else if (this.isModbus() || this.isModbusTCPdll() ||  this.isEthernetIp()) {
    ...
    isModbusTCPdll() {
		return (this.data.device.type === DeviceType.ModbusTCPDLL) ? true : false;
    }
