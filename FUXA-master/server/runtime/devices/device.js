...
var MODBUSclientdll = require('./modbusdll');
...
function Device(data, runtime) {
    ...
    } else if (data.type === DeviceEnum.ModbusTCPDLL) {
        if (!MODBUSclientdll) {
            return null;
        }
        comm = MODBUSclientdll.create(data, logger, events, manager);      
    ...
    this.connect = function () {
        ...
        if (data.type === DeviceEnum.ModbusRTU) {
            ...
            comm.init(MODBUSclientdll.ModbusTypes.TCP);
        }
...
function loadPlugin(type, module) {
    ...
    } else if (DeviceEnum.ModbusTCPDLL.startsWith(type)) {
        MODBUSclientdll = require(module);
